...
async function init() {
  ...
  const navItems=$$(".nav-list .nav-item");navItems[0].onclick=()=>showCrcView("overview");navItems[1].onclick=()=>showCrcView("returns");$("#crcWhatsappNav span:last-child").textContent="Abrir CRM";$("#crcWhatsappNav").onclick=async()=>{let release=Date.now();try{release=(await api("/api/release")).release||release;}catch{}const crm=window.open(`/central-crc/whatsapp?release=${encodeURIComponent(release)}`,"iea-crm-whatsapp");if(crm)crm.focus();};
