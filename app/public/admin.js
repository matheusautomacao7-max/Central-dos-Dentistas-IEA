...
$("#auditAppointments").addEventListener("click", async () => { try { const files = await Promise.all(state.appointmentFiles.map(async file => ({name: file.name, data: await fileToBase64(file)}))); const data = await api("/api/admin/import/appointments/audit", {method:"POST", body: JSON.stringify({files})}); state.appointmentToken = data.token; renderAppointmentPreview(data.preview); $("#adminImportResult").hidden = false; $("#adminImportResult").classList.toggle("error", !data.consolidated); $("#adminImportResult").innerHTML = `<strong>Auditoria concluída</strong><br>${data.read} atendimentos válidos · ${data.consolidated} pacientes consolidados · ${data.duplicates_removed} duplicados removidos${data.errors.length ? `<br><br><strong>Itens ignorados (não serão importados):</strong><br>${data.errors.map(escapeHtml).join("<br>")}` : ""}${data.consolidated ? "" : "<br><br><strong>Nenhum paciente válido foi encontrado para continuar.</strong>"}`; $("#validateAppointments").disabled = !data.consolidated; } catch(error) { showToast(error.message, true); } });
$("#validateAppointments").addEventListener("click", async () => { try { const data = await api("/api/admin/import/appointments/validate", {method:"POST", body: JSON.stringify({token: state.appointmentToken})}); renderAppointmentPreview(data.preview); $("#adminImportResult").hidden = false; const partial = data.can_confirm_partial === true; $("#adminImportResult").innerHTML = data.can_confirm ? `<strong>Validação concluída</strong><br>${data.total} pacientes prontos para importação.` : partial ? `<strong>Importação parcial disponível</strong><br>${data.valid} de ${data.total} pacientes podem ser importados agora.<br>Não serão importadas as carteiras: ${data.missing_professionals.map(escapeHtml).join(", ")}.` : `<strong>Validação pendente</strong><br>Nenhum paciente pode ser importado. Cadastre antes: ${data.missing_professionals.map(escapeHtml).join(", ")}.`; $("#confirmAppointments").dataset.partial = String(partial); $("#confirmAppointments").textContent = partial ? "3. Importar somente cadastrados" : "3. Confirmar importação"; $("#confirmAppointments").disabled = !(data.can_confirm || partial); } catch(error) { showToast(error.message, true); } });
$("#confirmAppointments").addEventListener("click", async () => { const allowPartial = $("#confirmAppointments").dataset.partial === "true"; const question = allowPartial ? "Importar somente as carteiras dos profissionais cadastrados e ignorar as demais?" : "Confirmar a importação das carteiras validadas?"; if (!confirm(question)) return; try { const data = await api("/api/admin/import/appointments/confirm", {method:"POST", body: JSON.stringify({token: state.appointmentToken, allow_partial: allowPartial})}); $("#adminImportResult").hidden = false; $("#adminImportResult").innerHTML = `<strong>Importação concluída</strong><br>${data.imported} novos pacientes · ${data.updated} últimas consultas atualizadas · ${data.scheduled || 0} próximas consultas programadas · ${data.unchanged} registros ignorados por já estarem na mesma data ou em uma data mais recente${data.skipped ? ` · ${data.skipped} pacientes não importados por pertencerem a: ${data.skipped_professionals.map(escapeHtml).join(", ")}` : ""}.`; $("#confirmAppointments").disabled = true; await loadAdmin(); } catch(error) { showToast(error.message, true); } });
