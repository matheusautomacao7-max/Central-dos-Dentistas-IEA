...
if (!template.includes("CRM_START_CONVERSATION_STABLE_V2") && !template.includes("CRM_CONVERSATION_RELOAD_RESUME_V4") && !template.includes("CRM_CONVERSATION_AUTO_REVEAL_V5") && !template.includes("CRM_REFRESH_RACE_FIXED_V9")) throw new Error("Fluxo estável de abertura de conversa ausente.");
if (!template.includes("CRM_CONVERSATION_ID_NORMALIZED_V3")) throw new Error("Normalização do ID da conversa ausente.");

if (!template.includes("CRM_CONVERSATION_AUTO_REVEAL_V5") && !template.includes("CRM_REFRESH_RACE_FIXED_V9")) throw new Error("Automatic conversation reveal missing.");
...
if (!template.includes("CRM_INTERNAL_TEAM_FILTER_V14")) throw new Error("Internal team Inbox filter missing.");
if (!template.includes("CRM_COMPOSER_TOOLS_V15")) throw new Error("Emoji and attachment composer tools missing.");
