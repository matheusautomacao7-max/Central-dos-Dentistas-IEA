...
assert.match(crmHtml, /crm-goals\.js\?v=20260804-attendance-complete-v2/);
assert.doesNotMatch(goalScript, /O pagamento é estimado: abaixo do mínimo de pagamento/);
...
try {
  ...
  await page.evaluate(() => window.IEACrmGoals.celebrateAttendanceCompletion());
  await page.getByText("Atendimento finalizado", { exact: true }).waitFor();
  assert.equal(await page.locator(".iea-attendance-complete").count(), 1);
  assert.equal(await page.locator(".iea-attendance-complete-card").count(), 1);
  assert.equal(await page.locator(".iea-attendance-complete-icon svg").count(), 1);
  assert.equal(await page.locator(".iea-attendance-complete").evaluate(element => getComputedStyle(element).transitionDuration), "0s");
  await page.setViewportSize({ width: 390, height: 844 });
